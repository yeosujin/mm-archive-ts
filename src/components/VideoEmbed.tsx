...
const VideoEmbed = memo(({ url, icon, iconText, platformName, thumbnailUrl, className = '', priority = false, autoplayInView = false, hideControls = false, initialUnmuted = false }: Props) => {
  ...
  if (videoType === 'r2') {
    return <VideoPlayer videoUrl={url} thumbnailUrl={thumbnailUrl} className={className} priority={priority} autoplayInView={autoplayInView} hideControls={hideControls} initialUnmuted={initialUnmuted} />;
  }
