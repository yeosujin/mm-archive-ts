...
- [[daily-tweet-bot]]이 "그 해 오늘"에 걸린 에피소드를 **첨부 → 본문 이미지** 순으로 트윗에 올린다.
  같은 URL이 양쪽에 있으면 한 번만 실린다
- **첨부가 비어 있으면 게시하지 않는다.** 첨부가 곧 "트윗에 올린다"는 표시다 —
  안 그러면 본문 이미지가 있는 과거 DM이 전부 자동 게시된다
- 에피소드 1개 = 트윗 1개(`groupKey`가 에피소드 id). 미디어가 4개를 넘으면 스레드로 이어진다
